...
public class Enemy implements Entity {
	...
	public void update() {
		//Check if its the first time this class is updated, if so do nothing
		if (first)
			first = false;
		else {
	...
	private Checkpoint findNextCheckpoint(Tile s, int[] dir) {
		 ...
		 boolean found = false;
		 //Integer to increment every loop
		 int counter = 1;
		 ...
		 while (!found) {
			 if (s.getXPlace() + dir[0] * counter == grid.getTilesWide() ||
				 ...
				 found = true;
				 //Move counter back 1 to find the tile before the new TileType
				 counter -= 1;
